...
export const register = async (phone, username, password, code) => {
    const response = await api.post('/auth/register', { phone, username, password, code });
    return response.data;
};
export const sendCode = async (phone, purpose) => {
...
// 获取会员信息
export const getMembership = async (deviceId) => {
    const response = await api.get(`/user/membership?deviceId=${deviceId}`);
    return response.data;
};
// 获取剩余次数
export const getRemainingEdits = async (deviceId) => {
    const response = await api.get(`/user/remaining-edits?deviceId=${deviceId}`);
    return response.data;
};
// 文案润色
export const polishContent = async (content, style) => {
...
// 获取积分记录
export const getCreditRecords = async (userId) => {
    const response = await api.get(`/user/points`, {
        headers: { 'x-user-id': userId }
    });
...
// 激活免费码
export const activateFreeCode = async (userId, code) => {
