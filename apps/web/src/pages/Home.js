...
const Home = () => {
    ...
    return (<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {/* Quota Cards */}
      {user && (<div className="grid grid-cols-3 gap-4 mb-8 max-w-2xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
          {!user ? (<div className="text-center py-8">
              ...
              <div className="flex justify-center space-x-4">
                <Link to="/login" className="px-6 py-2 border-2 border-indigo-600 text-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors">
                  登录
                </Link>
                <Link to="/register" className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                  注册
                </Link>
              </div>
            </div>) : (<>
          ...
          {error && (<div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm">
              {error}
            </div>)}
        </div>
        ...
        <div>
          ...
          {result ? (<div className="space-y-4">
              {result.imageUrls?.map((url, i) => (<ResultCard key={`img-${i}`} type="image" url={url}/>))}
              {result.texts?.map((text, i) => (<ResultCard key={`text-${i}`} type="text" content={text}/>))}
              {result.videoUrl && (<ResultCard type="video" url={result.videoUrl}/>)}
            </div>) : (<div className="bg-gray-100 rounded-xl p-8 text-center text-gray-500">
              暂无创作结果
            </div>)}
        </div>
      </div>
