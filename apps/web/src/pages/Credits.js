    ...
    return (<div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl p-8 text-white mb-8">
        <div className="flex items-center justify-between">
          ...
          <button onClick={handleRecharge} className="px-6 py-3 bg-white text-indigo-600 rounded-xl font-medium hover:bg-indigo-50 transition-colors">
            充值积分
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
        ...
        {loading ? (<p className="text-center text-gray-500 py-8">加载中...</p>) : records.length > 0 ? (<div className="space-y-4">
            {records.map((record) => (<div key={record.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-xl">
                <div>
          ...
          </div>) : (<p className="text-center text-gray-500 py-8">暂无积分记录</p>)}
      </div>
    </div>);
};
