...
const getCurrentParams = (rawParams) => {
    const payload = rawParams[PAYLOAD_KEY];
    if (payload && typeof payload === 'string') {
        const decoded = deserializeParams(payload);
...
const deserializeParams = (payload) => {
    if (!payload || typeof payload !== 'string') {
        return null;
    }
    try {
...
export function useSafeRouter() {
    ...
    const rawParams = useExpoParams();
    const push = (pathname, params = {}) => {
...
export function useSafeSearchParams() {
    ...
    const decodedParams = useMemo(() => {
        return getCurrentParams(rawParams);
    }, [rawParams]);
