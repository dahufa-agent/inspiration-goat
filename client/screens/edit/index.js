...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, ScrollView, Image, ActivityIndicator, Alert, KeyboardAvoidingView, Platform, Dimensions, Modal, } from "react-native";
...
export default function EditScreen() {
    ...
    const saveImageToGallery = async (imageUrl, filename) => {
        ...
        try {
            ...
            // 下载图片到临时目录
            const fileUri = `${FileSystem.cacheDirectory}${filename || `image_${Date.now()}.png`}`;
            // @ts-ignore
            ...
            const album = albums.find((a) => a.title === "灵感山羊");
            if (album) {
    ...
    // 保存视频到相册
    const saveVideoToGallery = async (videoUrl, filename) => {
        if (savingVideo)
            return;
        setSavingVideo(true);
        try {
            ...
            // 下载视频到临时目录
            const fileUri = `${FileSystem.cacheDirectory}${filename || `video_${Date.now()}.mp4`}`;
            // @ts-ignore
    ...
    // 长按保存图片
    const handleLongPressImage = (imageUrl, index) => {
        Alert.alert("保存图片", "选择保存方式", [
            {
                text: "保存到相册",
    ...
    // 长按保存视频
    const handleLongPressVideo = (videoUrl) => {
        Alert.alert("保存视频", "选择保存方式", [
            {
                text: "保存到相册",
    ...
    // 重新生成单条文案（不计入限制）
    const regenerateSingleText = async (index) => {
        setLoadingText(true);
        try {
    ...
    const regenerateVideo = async () => {
        ...
        if (selectedDuration === "free" && remainingVideoEdits <= 0) {
            Alert.alert("免费次数已用完", "今日5秒内视频免费编辑次数已用完，请明天再来或选择更长时长", [
    ...
    const selectedDurationInfo = DURATION_OPTIONS.find((o) => o.type === selectedDuration) || DURATION_OPTIONS[0];
    return (<Screen>
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
        ...
        ]} onPress={regenerateAllImages} disabled={loadingImage || remainingImages < LIMITS.images.perBatch}>
                {loadingImage ? (<ActivityIndicator size="small" color="#4F46E5"/>) : (<Text style={[
            ...
            {imageUrls.length > 0 && (<TouchableOpacity style={styles.mainImageContainer} onPress={() => setImagePreviewVisible(true)} activeOpacity={0.9}>
                <Image source={{ uri: imageUrls[selectedImageIndex] }} style={styles.mainImage} resizeMode="cover"/>
            ...
            <View style={styles.imageGrid}>
              {imageUrls.map((url, index) => (<TouchableOpacity key={`${url}-${index}`} style={[
                styles.imageItem,
                selectedImageIndex === index && styles.imageItemSelected,
            ]} onPress={() => handleSelectImage(index)} onLongPress={() => handleLongPressImage(url, index)} delayLongPress={500}>
                  <Image source={{ uri: url }} style={styles.imageThumb}/>
                  {selectedImageIndex === index && (<View style={styles.selectedBadge}>
          ...
          <View style={styles.section}>
        ...
        ]} onPress={regenerateAllTexts} disabled={loadingText || remainingTexts < LIMITS.texts.perBatch}>
                {loadingText ? (<ActivityIndicator size="small" color="#4F46E5"/>) : (<Text style={[
                styles.regenerateAllText,
            ...
            <View style={styles.textList}>
              {texts.map((t, index) => (<TouchableOpacity key={index} style={[
                styles.textItem,
                selectedTextIndex === index && styles.textItemSelected,
                  ...
                  <View style={styles.textItemHeader}>
                    ...
                    <TouchableOpacity onPress={() => regenerateSingleText(index)} disabled={loadingText}>
                      <Text style={styles.textRefreshText}>R</Text>
                    </TouchableOpacity>
                  </View>
                  <Text style={styles.textItemContent} numberOfLines={selectedTextIndex === index ? undefined : 2}>
                    {t}
                  </Text>
                </TouchableOpacity>))}
            </View>
            ...
            {/* Edit Selected Text */}
            {texts.length > 0 && (<View style={styles.textEditor}>
                <Text style={styles.textEditorLabel}>编辑当前文案</Text>
                <View style={styles.textCard}>
                  <TextInput style={styles.textInput} value={text} onChangeText={handleEditText} multiline placeholder="在这里编辑你的文案..." placeholderTextColor="#9CA3AF"/>
                </View>
              </View>)}
          </View>
          ...
          <View style={styles.section}>
            ...
            ]} onPress={() => setSelectedDuration(option.type)} disabled={loadingVideo}>
                  <Text style={[
            ...
            {selectedDuration === "free" && remainingVideoEdits <= 0 && (<Text style={styles.quotaWarning}>
        ...
        ]} onPress={regenerateVideo} disabled={loadingVideo || (selectedDuration === "free" && remainingVideoEdits <= 0)}>
              {loadingVideo ? (<ActivityIndicator color="#4F46E5"/>) : (<Text style={styles.regenerateVideoText}>
                  生成 {selectedDurationInfo.duration}秒 视频
                </Text>)}
            </TouchableOpacity>

            {videoUrl ? (<>
                <TouchableOpacity style={styles.videoContainer} onLongPress={() => handleLongPressVideo(videoUrl)} delayLongPress={500}>
                  <Video ref={videoRef} source={{ uri: videoUrl }} style={styles.video} useNativeControls resizeMode={ResizeMode.CONTAIN} isLooping/>
                </TouchableOpacity>
                <TouchableOpacity style={styles.videoSaveButtonBelow} onPress={() => saveVideoToGallery(videoUrl, `灵感山羊_视频_${Date.now()}.mp4`)} disabled={savingVideo}>
                  {savingVideo ? (<ActivityIndicator color="#4F46E5" size="small"/>) : (<Text style={styles.videoSaveTextBelow}>保存视频</Text>)}
                </TouchableOpacity>
              </>) : lastFrameUrl ? (<TouchableOpacity style={styles.mainImageContainer} onLongPress={() => handleLongPressImage(lastFrameUrl, 0)} delayLongPress={500}>
              ...
              </TouchableOpacity>) : (<View style={styles.videoPlaceholder}>
        ...
        <View style={styles.bottomAction}>
          <TouchableOpacity style={styles.saveButton} onPress={() => Alert.alert("提示", "内容已保存")}>
        ...
        <Modal visible={imagePreviewVisible} transparent animationType="fade" onRequestClose={() => setImagePreviewVisible(false)}>
          <View style={styles.modalContainer}>
            <TouchableOpacity style={styles.modalClose} onPress={() => setImagePreviewVisible(false)}>
              <Text style={styles.modalCloseText}>X</Text>
            </TouchableOpacity>
            <Image source={{ uri: imageUrls[selectedImageIndex] }} style={styles.previewImage} resizeMode="contain"/>
            <View style={styles.previewNav}>
              <TouchableOpacity style={styles.previewNavBtn} onPress={() => setSelectedImageIndex(Math.max(0, selectedImageIndex - 1))} disabled={selectedImageIndex === 0}>
                <Text style={styles.previewNavText}>{"<"}</Text>
              </TouchableOpacity>
              <Text style={styles.previewIndex}>
                {selectedImageIndex + 1} / {imageUrls.length}
              </Text>
              <TouchableOpacity style={styles.previewNavBtn} onPress={() => setSelectedImageIndex(Math.min(imageUrls.length - 1, selectedImageIndex + 1))} disabled={selectedImageIndex === imageUrls.length - 1}>
                <Text style={styles.previewNavText}>{">"}</Text>
              </TouchableOpacity>
            </View>
          </View>
