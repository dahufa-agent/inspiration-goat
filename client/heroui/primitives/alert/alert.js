...
// --------------------------------------------------
const Root = forwardRef(({ asChild, id, status = 'default', ...viewProps }, ref) => {
    const generatedId = useId();
    const nativeID = id != null ? String(id) : generatedId;
    const Component = asChild ? Slot.View : View;
    return (<AlertContext.Provider value={{ nativeID, status }}>
        <Component ref={ref} role="alert" aria-labelledby={`${nativeID}_label`} aria-describedby={`${nativeID}_desc`} nativeID={nativeID} {...viewProps}/>
...
// --------------------------------------------------
const Title = forwardRef(({ asChild, ...props }, ref) => {
    const { nativeID } = useRootContext();
    const Component = asChild ? Slot.Text : RNText;
    return (<Component ref={ref} role="heading" aria-level={2} nativeID={`${nativeID}_label`} {...props}/>);
});
Title.displayName = 'HeroUINative.Primitive.Alert.Title';
// --------------------------------------------------
const Description = forwardRef(({ asChild, ...props }, ref) => {
    const { nativeID } = useRootContext();
    const Component = asChild ? Slot.Text : RNText;
