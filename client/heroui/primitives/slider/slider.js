...
import { clamp, denormalizeValue, formatValue, normalizeValue, snapToStep, valueToPercent, } from './slider.utils';
// ---------------------------------------------------------------------------
// Slider context – provides all value logic to sub-components
// ---------------------------------------------------------------------------
const [SliderProvider, useSlider] = createContext({
    name: 'SliderPrimitiveContext',
    errorMessage: 'Slider compound components cannot be rendered outside the Slider.Root component',
});
// --------------------------------------------------
// Root – manages value state and provides context
// --------------------------------------------------
const Root = forwardRef(({ asChild, children, 'value': valueProp, defaultValue, onChange, onChangeEnd, minValue = 0, maxValue = 100, step = 1, formatOptions, orientation = 'horizontal', isDisabled = false, 'aria-label': ariaLabel, ...restProps }, ref) => {
    /** Track whether the consumer originally passed an array value */
    const wasArray = useRef(Array.isArray(valueProp) || Array.isArray(defaultValue));
    ...
    const values = useMemo(() => normalizeValue(internalValue ?? minValue).map((v) => clamp(v, minValue, maxValue)), [internalValue, minValue, maxValue]);
    // -----------------------------------------------------------------
    ...
    // Value helpers
    // -----------------------------------------------------------------
    const getThumbPercent = useCallback((index) => {
        const val = values[index] ?? minValue;
        return valueToPercent(val, minValue, maxValue);
    }, [values, minValue, maxValue]);
    const getThumbValueLabel = useCallback((index) => {
        const val = values[index] ?? minValue;
     ...
     */
    const getThumbMinValue = useCallback((index) => {
     ...
     */
    const getThumbMaxValue = useCallback((index) => {
     ...
     */
    const updateValue = useCallback((index, newValue) => {
    ...
    // -----------------------------------------------------------------
    const isThumbDragging = useCallback((index) => {
        return draggingIndex === index;
    }, [draggingIndex]);
    const setThumbDragging = useCallback((index, dragging) => {
     ...
     */
    const handleTapAtValue = useCallback((targetValue) => {
        if (isDisabled)
            return;
    ...
    return (<SliderProvider value={contextValue}>
        <Component ref={ref} aria-label={ariaLabel} aria-disabled={isDisabled} accessibilityState={{ disabled: isDisabled }} {...restProps}>
          {children}
        </Component>
      </SliderProvider>);
});
...
// --------------------------------------------------
const Track = forwardRef(({ asChild, children, onLayout, ...restProps }, ref) => {
    const { values, orientation, isDisabled, getThumbValueLabel, setTrackSize, } = useSlider();
    const state = useMemo(() => ({ values, getThumbValueLabel }), [values, getThumbValueLabel]);
    const renderProps = { state, orientation, isDisabled };
    ...
    const Component = asChild ? Slot.View : View;
    return (<Component ref={ref} onLayout={handleLayout} {...restProps}>
        {resolvedChildren}
      </Component>);
});
...
// --------------------------------------------------
const Fill = forwardRef(({ asChild, ...props }, ref) => {
    const Component = asChild ? Slot.View : View;
...
// --------------------------------------------------
const Thumb = forwardRef(({ asChild, index = 0, onLayout, ...props }, ref) => {
    const { orientation, isDisabled, getThumbPercent, getThumbValueLabel, setThumbSize, } = useSlider();
    const Component = asChild ? Slot.View : View;
    /**
    ...
    return (<Component ref={ref} role="slider" aria-disabled={isDisabled} accessibilityValue={{
            min: 0,
            max: 100,
            now: percentNow,
            text: getThumbValueLabel(index),
        }} accessibilityState={{ disabled: isDisabled }} onLayout={handleLayout} {...props}/>);
});
Thumb.displayName = 'HeroUINative.Primitive.Slider.Thumb';
// --------------------------------------------------
// Output – resolves render-function children via context
// --------------------------------------------------
const Output = forwardRef(({ asChild, children, ...restProps }, ref) => {
