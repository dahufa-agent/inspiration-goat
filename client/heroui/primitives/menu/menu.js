...
import { useAugmentedRef, useControllableState, useRelativePosition, } from '../../helpers/internal/hooks';
...
const Root = forwardRef(({ asChild, isOpen: isOpenProp, isDefaultOpen, onOpenChange: onOpenChangeProp, isDisabled, presentation = 'popover', ...viewProps }, ref) => {
...
// --------------------------------------------------
const Trigger = forwardRef(({ asChild, onPress: onPressProp, isDisabled = false, ...props }, ref) => {
    const { onOpenChange, isOpen, isDisabled: isDisabledRoot, setTriggerPosition, setContentLayout, isDefaultOpen, triggerPosition, } = useRootContext();
    ...
    return (<Component ref={augmentedRef} aria-disabled={isDisabledValue} role="button" onPress={onPress} disabled={isDisabledValue} {...props}/>);
});
// --------------------------------------------------
/**
...
function Portal({ forceMount, hostName, children }) {
    ...
    return (<PrimitivePortal hostName={hostName} name={`${value.nativeID}_menu_portal`}>
      <RootContext.Provider value={value}>{children}</RootContext.Provider>
    </PrimitivePortal>);
}
// --------------------------------------------------
const Overlay = forwardRef(({ asChild, forceMount, onPress: OnPressProp, closeOnPress = true, ...props }, ref) => {
    const { isOpen, onOpenChange, setTriggerPosition, setContentLayout } = useRootContext();
    function onPress(ev) {
        if (closeOnPress) {
            onOpenChange(false);
            setTriggerPosition(null);
 ...
 */
const Content = forwardRef(({ asChild = false, forceMount, align = 'start', placement = 'bottom', offset = 0, alignOffset = 0, avoidCollisions = true, onLayout: onLayoutProp, insets, style, disablePositioningStyle, width = 'content-fit', ...props }, ref) => {
    const { isOpen, onOpenChange, contentLayout, nativeID, setContentLayout, setTriggerPosition, triggerPosition, } = useRootContext();
    useEffect(() => {
    ...
    const Component = asChild ? Slot.View : View;
    return (<Component ref={ref} role="menu" nativeID={nativeID} aria-modal={true} style={[positionStyle, widthStyle, style]} onLayout={onLayout} {...props}/>);
});
// --------------------------------------------------
...
// Group
// --------------------------------------------------
const Group = forwardRef(({ asChild, selectionMode = 'none', selectedKeys: selectedKeysProp, defaultSelectedKeys, onSelectionChange: onSelectionChangeProp, disabledKeys: disabledKeysProp, isDisabled = false, shouldCloseOnSelect, ...viewProps }, ref) => {
...
// Item
// --------------------------------------------------
const Item = forwardRef(({ asChild, id: itemId, isDisabled: isDisabledProp = false, shouldCloseOnSelect: shouldCloseOnSelectProp, isSelected: isSelectedProp, onSelectedChange, onPress: onPressProp, variant = 'default', ...props }, ref) => {
    ...
    const onPress = useCallback((ev) => {
        if (effectiveDisabled)
            return;
        if (isInsideGroup && itemId !== undefined) {
            const { selectionMode, selectedKeys, onSelectionChange } = groupContext;
    ...
    const Component = asChild ? Slot.Pressable : Pressable;
    return (<ItemContext.Provider value={itemContextValue}>
        <Component ref={ref} role={role} aria-checked={isCheckable ? isSelected : undefined} aria-disabled={effectiveDisabled} disabled={effectiveDisabled} accessibilityState={{
...
// ItemTitle
// --------------------------------------------------
const ItemTitle = forwardRef(({ asChild, ...props }, ref) => {
    useItemContext();
...
// ItemDescription
// --------------------------------------------------
const ItemDescription = forwardRef(({ asChild, ...props }, ref) => {
    useItemContext();
    const Component = asChild ? Slot.Text : Text;
    return <Component ref={ref} {...props}/>;
});
...
// ItemIndicator
// --------------------------------------------------
const ItemIndicator = forwardRef(({ asChild, forceMount, ...props }, ref) => {
    const { isSelected } = useItemContext();
    if (!forceMount && !isSelected) {
        return null;
    }
    const Component = asChild ? Slot.View : View;
    return (<Component ref={ref} role="presentation" aria-hidden={!isSelected} {...props}/>);
});
// --------------------------------------------------
// Label
// --------------------------------------------------
