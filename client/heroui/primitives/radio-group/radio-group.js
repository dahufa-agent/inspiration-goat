...
const Root = forwardRef(({ asChild, value, onValueChange, isDisabled = false, isInvalid = false, variant, ...viewProps }, ref) => {
...
// --------------------------------------------------
const Item = forwardRef(({ asChild, value: itemValue, isDisabled: disabledProp = false, onPress: onPressProp, ...props }, ref) => {
    const { isDisabled, value, onValueChange } = useRadioGroupContext();
    function onPress(ev) {
        if (isDisabled || disabledProp)
            return;
        onValueChange(itemValue);
        onPressProp?.(ev);
    }
    const Component = asChild ? Slot.Pressable : Pressable;
    return (<Component ref={ref} role="radio" onPress={onPress} aria-checked={value === itemValue} disabled={(isDisabled || disabledProp) ?? false} accessibilityState={{
