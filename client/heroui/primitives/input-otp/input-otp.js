...
function useInputOTPContext() {
    ...
    if (!context) {
        throw new Error('InputOTP compound components cannot be rendered outside the InputOTP component');
    }
    return context;
}
// --------------------------------------------------
const Root = forwardRef(({ maxLength, onComplete, isDisabled = false, isInvalid = false, variant, pattern, inputMode = 'numeric', pasteTransformer, value: valueProp, defaultValue, onChange, placeholder, placeholderTextColor, placeholderTextClassName, children, onFocus: onFocusProp, onBlur: onBlurProp, textInputProps, style, className, }, ref) => {
    ...
    const onChangeText = useCallback((text) => {
        ...
        const transformedText = isPaste && pasteTransformFn ? pasteTransformFn(text) : text;
    ...
    // Render
    // --------------------------------------------------
    return (<InputOTPContext.Provider value={contextValue}>
        <Pressable onPress={focus} disabled={isDisabled} accessible={false} accessibilityRole="none" style={style} className={className}>
          {children}
        ...
        ]} value={value} onChangeText={onChangeText} onFocus={onFocus} onBlur={onBlur} maxLength={maxLength} inputMode={inputMode} 
    /**
     * On iOS if the input has an opacity of 0, we can't paste text into it.
     * As we're setting the opacity to 0.02, we need to hide the caret.
     */
    caretHidden={Platform.OS === 'ios'} autoComplete={Platform.OS === 'android' ? 'sms-otp' : 'one-time-code'} autoCorrect={false} autoCapitalize="none" textContentType="oneTimeCode" accessible accessibilityRole="text" accessibilityLabel={`One-time passcode input, ${maxLength} digits`} accessibilityHint={`Enter ${maxLength} digit verification code`} accessibilityState={accessibilityState} accessibilityValue={accessibilityValue} editable={!isDisabled} {...textInputProps}/>
        </Pressable>
      </InputOTPContext.Provider>);
});
Root.displayName = 'HeroUINative.Primitive.InputOTP.Root';
// --------------------------------------------------
const Group = forwardRef(({ asChild, accessible = false, accessibilityRole = 'none', ...props }, ref) => {
    useInputOTPContext(); // Ensure we're inside InputOTP
    const Component = asChild ? SlotPrimitive.View : View;
    return (<Component ref={ref} accessible={accessible} accessibilityRole={accessibilityRole} {...props}/>);
});
Group.displayName = 'HeroUINative.Primitive.InputOTP.Group';
// --------------------------------------------------
const Slot = forwardRef(({ asChild, index, accessible = false, accessibilityRole = 'none', accessibilityLabel, ...props }, ref) => {
    const { slots, maxLength } = useInputOTPContext();
    if (index < 0 || index >= slots.length) {
        throw new Error(`InputOTP.Slot index ${index} is out of range. Must be between 0 and ${slots.length - 1}.`);
    }
    ...
    const computedAccessibilityLabel = accessibilityLabel ??
        (accessible && slot
            ...
            : undefined);
    const Component = asChild ? SlotPrimitive.View : View;
    return (<Component ref={ref} accessible={accessible} accessibilityRole={accessibilityRole} accessibilityLabel={computedAccessibilityLabel} {...props}/>);
});
Slot.displayName = 'HeroUINative.Primitive.InputOTP.Slot';
// --------------------------------------------------
const Separator = forwardRef(({ asChild, accessible = false, accessibilityRole = 'none', ...props }, ref) => {
    useInputOTPContext(); // Ensure we're inside InputOTP
    const Component = asChild ? SlotPrimitive.View : View;
    return (<Component ref={ref} accessible={accessible} accessibilityRole={accessibilityRole} {...props}/>);
});
