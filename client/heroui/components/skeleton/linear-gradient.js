...
const LinearGradientComponent = ({ colors = ['transparent', 'rgba(255, 255, 255, 0.5)', 'transparent'], }) => {
    const gradientId = colors.join('-');
    return (<Svg style={StyleSheet.absoluteFill}>
      <Defs>
        <LinearGradient id={gradientId} x1={0} y1={0.5} x2={1} y2={0.5}>
          {colors.map((color, index) => {
            ...
            return (<Stop key={index} offset={`${(index / (colors.length - 1)) * 100}%`} stopColor={processedColor} stopOpacity={opacity}/>);
        })}
        </LinearGradient>
      </Defs>
