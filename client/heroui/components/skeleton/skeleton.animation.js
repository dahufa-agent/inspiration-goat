...
import { cancelAnimation, Easing, FadeIn, FadeOut, interpolate, ReduceMotion, useAnimatedStyle, withRepeat, withTiming, } from 'react-native-reanimated';
...
import { createContext, getAnimationState, getAnimationValueProperty, getIsAnimationDisabledValue, getRootAnimationState, } from '../../helpers/internal/utils';
import { DEFAULT_EASING, DEFAULT_PULSE_DURATION, DEFAULT_PULSE_MAX_OPACITY, DEFAULT_PULSE_MIN_OPACITY, DEFAULT_SHIMMER_DURATION, DEFAULT_SPEED, } from './skeleton.constants';
const [SkeletonAnimationProvider, useSkeletonAnimation] = createContext({
...
export function useSkeletonRootAnimation(options) {
    ...
    const { isAnimationDisabled: isAnimationDisabledFromRoot } = getRootAnimationState(animation);
    const isAllAnimationsDisabledFromRoot = useCombinedAnimationDisabledState(animation);
    const isAllAnimationsDisabled = isAllAnimationsDisabledFromRoot || isAnimationDisabledFromRoot;
    ...
    const { animationConfig: enteringAnimationConfig, isAnimationDisabled: isEnteringAnimationDisabled, } = getAnimationState(enteringAnimation);
    const { animationConfig: exitingAnimationConfig, isAnimationDisabled: isExitingAnimationDisabled, } = getAnimationState(exitingAnimation);
    const { animationConfig: shimmerAnimationConfig } = getAnimationState(shimmerAnimation);
    const { animationConfig: pulseAnimationConfig } = getAnimationState(pulseAnimation);
...
export function useSkeletonShimmerAnimation(options) {
    ...
    const { componentWidth, offset, progress, screenWidth } = useSkeletonAnimation();
    ...
    const { animationConfig, isAnimationDisabled } = getAnimationState(shimmerAnimation);
    ...
    const defaultHighlightColor = useMemo(() => {
        return theme === 'dark'
            ? colorKit
                .setAlpha(colorKit.increaseBrightness(themeColorBackground, 10).hex(), 0.1)
                .hex()
            : colorKit
                .setAlpha(colorKit.decreaseBrightness(themeColorBackground, 10).hex(), 0.75)
                .hex();
    }, [theme, themeColorBackground]);
    const shimmerHighlightColor = animationConfig?.highlightColor ?? defaultHighlightColor;
    const gradientColors = useMemo(() => ['transparent', shimmerHighlightColor, 'transparent'], [shimmerHighlightColor]);
    const rContainerStyle = useAnimatedStyle(() => {
        ...
        const translateX = interpolate(progress.get(), [0, 1], [-(componentWidth + offset), screenWidth]);
        return {
...
export function useSkeletonPulseAnimation(options) {
    ...
    const { animationConfig, isAnimationDisabled } = getAnimationState(pulseAnimation);
    const isAnimationDisabledValue = getIsAnimationDisabledValue({
        isAnimationDisabled,
        isAllAnimationsDisabled,
    });
    ...
    const rContainerStyle = useAnimatedStyle(() => {
        ...
        const opacity = interpolate(progress.get(), [0, 1], [pulseMinOpacity, pulseMaxOpacity]);
        return {
            opacity,
        };
    });
