...
import { SkeletonAnimationProvider, useSkeletonAnimation, useSkeletonPulseAnimation, useSkeletonRootAnimation, useSkeletonShimmerAnimation, } from './skeleton.animation';
import { DISPLAY_NAME } from './skeleton.constants';
import { skeletonClassNames, skeletonStyleSheet } from './skeleton.styles';
// --------------------------------------------------
const ShimmerAnimation = ({ animation, isAnimatedStyleActive = true }) => {
    ...
    return (<Animated.View style={shimmerStyle}>
      <LinearGradientComponent colors={gradientColors}/>
    </Animated.View>);
};
// --------------------------------------------------
const PulseAnimation = ({ children, animation, isAnimatedStyleActive = true }) => {
...
const Skeleton = (props) => {
    const { children, isLoading = true, variant = 'shimmer', animation, isAnimatedStyleActive = true, className, style, ...restProps } = props;
    ...
    if (!isLoading) {
        return (<Animated.View key="content" entering={entering} exiting={exiting}>
        {children}
      </Animated.View>);
    }
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
      <SkeletonAnimationProvider value={animationContextValue}>
        <PulseAnimation animation={animation} isAnimatedStyleActive={isAnimatedStyleActive}>
          <Animated.View key="skeleton" entering={entering} exiting={exiting} onLayout={handleLayout} style={[skeletonStyleSheet.borderCurve, style]} className={rootClassName} {...restProps}>
            {variant === 'shimmer' && componentWidth > 0 && (<ShimmerAnimation animation={animation} isAnimatedStyleActive={isAnimatedStyleActive}/>)}
          </Animated.View>
        </PulseAnimation>
