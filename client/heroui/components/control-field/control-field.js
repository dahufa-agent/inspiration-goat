...
const ControlField = forwardRef((props, ref) => {
    const { children, className, isSelected, onSelectedChange, isDisabled = false, isInvalid = false, isRequired = false, onPressIn, onPressOut, animation, ...restProps } = props;
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
        <FormFieldProvider value={formFieldContextValue}>
          <ControlFieldProvider value={contextValue}>
            <Pressable ref={ref} className={rootClassName} onPress={handlePress} onPressIn={handlePressIn} onPressOut={handlePressOut} disabled={isDisabled} {...restProps}>
              {content}
            </Pressable>
...
const ControlFieldIndicator = forwardRef((props, ref) => {
    ...
    const enhancedChildren = useMemo(() => {
        ...
        if (variant === 'checkbox') {
            return (<Checkbox isSelected={isSelected} onSelectedChange={onSelectedChange} isDisabled={isDisabled} isInvalid={isInvalid}/>);
        }
        if (variant === 'radio') {
            return (<Radio isSelected={isSelected} onSelectedChange={onSelectedChange} isDisabled={isDisabled} isInvalid={isInvalid}/>);
        }
        return (<Switch isSelected={isSelected} onSelectedChange={onSelectedChange} isDisabled={isDisabled}/>);
    }, [
        children,
        variant,
        isSelected,
    ...
    return (<View ref={ref} className={indicatorClassName} {...restProps}>
        {enhancedChildren}
      </View>);
});
// --------------------------------------------------
