...
import { CheckboxAnimationProvider, useCheckboxIndicatorAnimation, useCheckboxRootAnimation, } from './checkbox.animation';
...
const CheckboxRoot = forwardRef((props, ref) => {
    const { children, isSelected, onSelectedChange, isDisabled = false, isInvalid = false, variant, hitSlop = DEFAULT_HIT_SLOP, className, style, onPressIn, onPressOut, animation, isAnimatedStyleActive = true, ...restProps } = props;
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
        <CheckboxAnimationProvider value={animationContextValue}>
          <AnimatedRootView ref={ref} className={rootClassName} isSelected={isSelected} onSelectedChange={onSelectedChange} isDisabled={isDisabled} isInvalid={isInvalid} hitSlop={hitSlop} onPressIn={handlePressIn} onPressOut={handlePressOut} style={rootStyle} {...restProps}>
            {content}
          </AnimatedRootView>
...
const CheckboxIndicator = forwardRef((props, ref) => {
    const { children, iconProps, className, style, animation, isAnimatedStyleActive = true, ...restProps } = props;
    ...
    const content = typeof children === 'function'
        ? children(renderProps)
        : (children ??
            (isAnimationDisabled ? (<View className="translate-y-px">
            <CheckIcon size={iconSize} color={iconColor}/>
          </View>) : (<AnimatedCheckIcon size={iconSize} strokeWidth={iconStrokeWidth} color={iconColor} isSelected={isSelected} enterDuration={iconEnterDuration} exitDuration={iconExitDuration}/>)));
    return (<AnimatedIndicatorView ref={ref} className={indicatorClassName} style={indicatorStyle} {...restProps}>
      {content}
