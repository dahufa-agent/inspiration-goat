...
import { BottomSheetContent, CheckIcon, FullWindowOverlay, HeroText, } from '../../helpers/internal/components';
import { AnimationSettingsProvider, useAnimationSettings, } from '../../helpers/internal/contexts';
import { usePopupOverlayAnimation, usePopupPopoverContentAnimation, usePopupRootAnimation, } from '../../helpers/internal/hooks';
import { childrenToString } from '../../helpers/internal/utils';
import * as MenuPrimitives from '../../primitives/menu';
import { CloseButton } from '../close-button';
import { MenuAnimationProvider, useMenuAnimation, useMenuItemAnimation, } from './menu.animation';
import { DEFAULT_ALIGN_OFFSET, DEFAULT_INSETS, DEFAULT_OFFSET, DISPLAY_NAME, } from './menu.constants';
...
// --------------------------------------------------
const MenuRoot = forwardRef(({ children, isOpen: isOpenProp, isDefaultOpen, onOpenChange: onOpenChangeProp, presentation = 'popover', animation, ...props }, ref) => {
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
        <MenuAnimationProvider value={animationContextValue}>
          <MenuPrimitives.Root ref={ref} presentation={presentation} isOpen={isOpenProp} isDefaultOpen={isDefaultOpen} onOpenChange={onOpenChangeProp} {...props}>
            {children}
          </MenuPrimitives.Root>
...
// --------------------------------------------------
const MenuPortal = ({ className, children, disableFullWindowOverlay = false, ...props }) => {
...
// --------------------------------------------------
const MenuOverlay = forwardRef(({ className, style, animation, isAnimatedStyleActive = true, ...props }, ref) => {
...
// --------------------------------------------------
const MenuContentPopover = forwardRef(({ placement = 'bottom', align = 'center', avoidCollisions = true, offset = DEFAULT_OFFSET, alignOffset = DEFAULT_ALIGN_OFFSET, className, children, style, animation, ...props }, ref) => {
    ...
    return (<MenuContentContext value={{ placement }}>
        {isReady && (<AnimatedContent ref={ref} entering={entering} exiting={exiting} placement={placement} align={align} avoidCollisions={avoidCollisions} offset={offset} alignOffset={alignOffset} insets={insets} className={contentClassName} style={[menuStyleSheet.borderCurve, style]} {...props}>
            {children}
          </AnimatedContent>)}
        <AnimatedContent placement={placement} accessible={false} accessibilityElementsHidden={true} importantForAccessibility="no" pointerEvents="none" collapsable={false} align={align} avoidCollisions={avoidCollisions} offset={offset} alignOffset={alignOffset} insets={insets} className={cn(contentClassName, 'absolute opacity-0')} style={[menuStyleSheet.borderCurve, style]} {...props}>
          {children}
        </AnimatedContent>
      </MenuContentContext>);
});
// --------------------------------------------------
const MenuContentBottomSheet = forwardRef(({ children, index: initialIndex, backgroundClassName, handleIndicatorClassName, contentContainerClassName: contentContainerClassNameProp, contentContainerProps, animation, animationConfigs, ...restProps }, ref) => {
...
const MenuContent = forwardRef((props, ref) => {
    ...
    if (props.presentation === 'bottom-sheet') {
        return (<MenuContentBottomSheet ref={ref} {...props}/>);
    }
    return (<MenuContentPopover ref={ref} {...props}/>);
});
// --------------------------------------------------
const MenuClose = forwardRef((props, ref) => {
...
const MenuLabel = forwardRef(({ className, children, ...props }, ref) => {
    ...
    return (<MenuPrimitives.Label ref={ref} className={labelClassName} {...props}>
        {children}
...
const MenuGroup = forwardRef(({ className, children, ...props }, ref) => {
    ...
    return (<MenuPrimitives.Group ref={ref} className={groupClassName} {...props}>
        {children}
      </MenuPrimitives.Group>);
});
// --------------------------------------------------
const MenuItemComponent = forwardRef(({ children, className, style, isDisabled = false, variant = 'default', animation, isAnimatedStyleActive = true, onPressIn, onPressOut, ...props }, ref) => {
    const { rItemStyle, isPressed, animationOnPressIn, animationOnPressOut } = useMenuItemAnimation({ animation, variant });
    ...
    const content = stringifiedChildren ? (<MenuItemTitle>{stringifiedChildren}</MenuItemTitle>) : (resolvedChildren);
    const itemStyle = isAnimatedStyleActive
        ? [menuStyleSheet.borderCurve, rItemStyle, style]
        : [menuStyleSheet.borderCurve, style];
    return (<AnimatedItem ref={ref} className={itemClassName} style={itemStyle} isDisabled={isDisabled} variant={variant} onPressIn={handlePressIn} onPressOut={handlePressOut} {...props}>
        {content}
      </AnimatedItem>);
});
// --------------------------------------------------
const MenuItemTitle = forwardRef(({ className, children, ...props }, ref) => {
    const { variant } = useMenuItem();
    const itemTitleClassName = menuClassNames.itemTitle({ className, variant });
    return (<HeroText ref={ref} accessibilityRole="text" className={itemTitleClassName} {...props}>
        {children}
      </HeroText>);
});
// --------------------------------------------------
const MenuItemDescription = forwardRef(({ className, children, ...props }, ref) => {
...
// --------------------------------------------------
const MenuItemIndicator = forwardRef(({ className, children, variant = 'checkmark', iconProps, forceMount = true, ...props }, ref) => {
    ...
    const defaultContent = variant === 'dot' ? (<View style={{
            width: iconSize,
            height: iconSize,
            borderRadius: iconSize / 2,
            backgroundColor: iconColor,
        }}/>) : (<CheckIcon size={iconSize} color={iconColor}/>);
    return (<MenuPrimitives.ItemIndicator ref={ref} className={itemIndicatorClassName} forceMount={forceMount} {...props}>
