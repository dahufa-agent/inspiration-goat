...
const Description = forwardRef((props, ref) => {
    const { children, className, nativeID, isInvalid: localIsInvalid, isDisabled: localIsDisabled, hideOnInvalid = false, animation, ...restProps } = props;
    ...
    if (isInvalid && hideOnInvalid)
        return null;
    return (<AnimatedText ref={ref} entering={entering} exiting={exiting} className={rootClassName} nativeID={nativeID} {...restProps}>
      {children}
    </AnimatedText>);
});
// --------------------------------------------------
