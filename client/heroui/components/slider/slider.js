...
const SliderRoot = forwardRef((props, ref) => {
    const { children, orientation = 'horizontal', isDisabled = false, animation, className, style, ...primitiveProps } = props;
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
      <SliderPrimitives.Root ref={ref} orientation={orientation} isDisabled={isDisabled} className={rootClassName} style={style} {...primitiveProps}>
        {children}
...
const SliderOutput = forwardRef((props, ref) => {
    ...
    const resolvedChildren = typeof children === 'function'
        ...
        : children;
    return (<SliderPrimitives.Output ref={ref} style={style} {...restProps}>
...
const SliderTrack = forwardRef((props, ref) => {
    ...
    const { minValue, maxValue, orientation, isDisabled, handleTapAtValue, trackSize, thumbSize, } = useSlider();
    ...
    const tapGesture = useMemo(() => {
        ...
        return Gesture.Tap()
            ...
            .enabled(!isDisabled)
            .onEnd((event) => {
            if (effectiveTrackSize <= 0)
                return;
            const pos = orientation === 'horizontal' ? event.x : event.y;
            const adjustedPos = orientation === 'horizontal'
                ? pos - thumbSize / 2
    ...
    return (<GestureDetector gesture={tapGesture}>
      <SliderPrimitives.Track ref={ref} className={trackClassName} style={[styleSheet.borderCurve, style]} hitSlop={hitSlop} {...restProps}>
        {children}
      </SliderPrimitives.Track>
    </GestureDetector>);
});
// --------------------------------------------------
const SliderFill = forwardRef((props, ref) => {
    ...
    return (<SliderPrimitives.Fill ref={ref} className={fillClassName} style={[styleSheet.borderCurve, fillStyle, style]} {...restProps}/>);
});
// --------------------------------------------------
const SliderThumb = forwardRef((props, ref) => {
    const { index = 0, isDisabled: thumbDisabled, animation, className, classNames, styles: stylesProp, style, hitSlop = 12, onTouchEnd, children, ...restProps } = props;
    const { values, minValue, maxValue, step, orientation, isDisabled: sliderDisabled, getThumbPercent, isThumbDragging, updateValue, setThumbDragging, trackSize, thumbSize, } = useSlider();
    ...
    const { thumbContainer: containerSlot, thumbKnob: knobSlot } = sliderClassNames.thumb({ orientation });
    ...
    const panGesture = useMemo(() => {
        ...
        const gesture = Gesture.Pan()
            ...
            const delta = orientation === 'horizontal'
                ? event.translationX
                : -event.translationY;
            const valueDelta = effectiveTrackSize > 0
    ...
    return (<GestureDetector gesture={panGesture}>
      <SliderPrimitives.Thumb ref={ref} index={index} className={thumbContainerClassName} style={[
            styleSheet.borderCurve,
            positionStyle,
            stylesProp?.thumbContainer,
            style,
        ]} onTouchEnd={handleTouchEnd} hitSlop={hitSlop} {...restProps}>
        {children ?? (<AnimatedKnob className={thumbKnobClassName} style={[styleSheet.borderCurve, rKnobStyle, stylesProp?.thumbKnob]}/>)}
