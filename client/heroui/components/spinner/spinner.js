...
import { createContext, getElementWithDefault, } from '../../helpers/internal/utils';
...
const SpinnerRoot = forwardRef((props, ref) => {
    const { children, size = 'md', color = 'default', isLoading = true, className, animation, ...restProps } = props;
    ...
    const indicatorElement = useMemo(() => getElementWithDefault(children, DISPLAY_NAME.INDICATOR, <SpinnerIndicator />), [children]);
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
      <SpinnerProvider value={contextValue}>
        <AnimatedRoot ref={ref} entering={entering} exiting={exiting} isLoading={isLoading} className={rootClassName} {...restProps}>
...
// --------------------------------------------------
const SpinnerIndicator = forwardRef((props, ref) => {
    const { children, className, style, iconProps, animation, isAnimatedStyleActive = true, ...restProps } = props;
    ...
    return (<AnimatedIndicator ref={ref} className={indicatorClassName} style={indicatorStyle} {...restProps}>
        {children || (<SpinnerIcon width={iconProps?.width ?? iconSize} height={iconProps?.height ?? iconSize} color={iconProps?.color ?? iconColor}/>)}
      </AnimatedIndicator>);
});
// --------------------------------------------------
