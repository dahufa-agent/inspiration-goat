...
import { PopoverAnimationProvider, usePopoverAnimation, } from './popover.animation';
import { DEFAULT_ALIGN_OFFSET, DEFAULT_INSETS, DEFAULT_OFFSET, DISPLAY_NAME, } from './popover.constants';
...
const PopoverContentContext = createContext({
    placement: undefined,
});
// --------------------------------------------------
const PopoverRoot = forwardRef(({ children, isOpen: isOpenProp, isDefaultOpen, onOpenChange: onOpenChangeProp, presentation = 'popover', animation, ...props }, ref) => {
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
        <PopoverAnimationProvider value={animationContextValue}>
          <PopoverPrimitives.Root ref={ref} presentation={presentation} isOpen={isOpenProp} isDefaultOpen={isDefaultOpen} onOpenChange={onOpenChangeProp} {...props}>
            {children}
...
// --------------------------------------------------
const PopoverPortal = ({ className, children, disableFullWindowOverlay = false, ...props }) => {
    const animationSettingsContext = useAnimationSettings();
    const animationContext = usePopoverAnimation();
    const portalClassName = popoverClassNames.portal({ className });
    return (<PopoverPrimitives.Portal {...props}>
...
// --------------------------------------------------
const PopoverOverlay = forwardRef(({ className, style, animation, isAnimatedStyleActive = true, ...props }, ref) => {
...
// --------------------------------------------------
const PopoverContentPopover = forwardRef(({ placement = 'bottom', align = 'center', avoidCollisions = true, offset = DEFAULT_OFFSET, alignOffset = DEFAULT_ALIGN_OFFSET, className, children, style, animation, ...props }, ref) => {
    ...
    return (<PopoverContentContext value={{ placement }}>
        {isReady && (<AnimatedContent ref={ref} entering={entering} exiting={exiting} placement={placement} align={align} avoidCollisions={avoidCollisions} offset={offset} alignOffset={alignOffset} insets={insets} className={contentClassName} style={[popoverStyleSheet.contentContainer, style]} {...props}>
            {children}
          </AnimatedContent>)}
        <AnimatedContent placement={placement} accessible={false} accessibilityElementsHidden={true} importantForAccessibility="no" pointerEvents="none" collapsable={false} align={align} avoidCollisions={avoidCollisions} offset={offset} alignOffset={alignOffset} insets={insets} className={cn(contentClassName, 'absolute opacity-0')} style={[popoverStyleSheet.contentContainer, style]} {...props}>
          {children}
        </AnimatedContent>
      </PopoverContentContext>);
});
// --------------------------------------------------
const PopoverContentBottomSheet = forwardRef(({ children, index: initialIndex, backgroundClassName, handleIndicatorClassName, contentContainerClassName: contentContainerClassNameProp, contentContainerProps, animation, animationConfigs, ...restProps }, ref) => {
...
const PopoverContent = forwardRef((props, ref) => {
    ...
    if (props.presentation === 'bottom-sheet') {
        return (<PopoverContentBottomSheet ref={ref} {...props}/>);
    }
    return (<PopoverContentPopover ref={ref} {...props}/>);
});
// --------------------------------------------------
...
// --------------------------------------------------
const PopoverTitle = forwardRef(({ className, children, ...props }, ref) => {
    const titleClassName = popoverClassNames.label({ className });
    return (<HeroText ref={ref} role="heading" accessibilityRole="header" className={titleClassName} {...props}>
        {children}
      </HeroText>);
});
// --------------------------------------------------
const PopoverDescription = forwardRef(({ className, children, ...props }, ref) => {
    const descriptionClassName = popoverClassNames.description({
        className,
    });
    return (<HeroText ref={ref} accessibilityRole="text" className={descriptionClassName} {...props}>
        {children}
      </HeroText>);
});
// --------------------------------------------------
const PopoverArrow = forwardRef(({ children, style, className, height = 12, width = 20, fill, stroke, strokeWidth = 1, placement: placementLocal, strokeBaselineInset = 1, }, ref) => {
    ...
    return (<Animated.View ref={ref} className={arrowClassName} style={[arrowPositionStyle, style]} pointerEvents="none">
        {children ? (children) : (<ArrowSvg width={width} height={height} placement={placement} fill={arrowFill} stroke={arrowStroke} strokeWidth={strokeWidth}/>)}
      </Animated.View>);
});
// --------------------------------------------------
