...
const FieldErrorRoot = forwardRef((props, ref) => {
    const { children, className, classNames, style, styles, textProps, isInvalid: localIsInvalid, animation, ...restProps } = props;
    ...
    const renderedChildren = stringifiedChildren ? (<HeroText className={textClassName} style={styles?.text} {...textProps}>
        {stringifiedChildren}
      </HeroText>) : (children);
    return (<Animated.View ref={ref} entering={entering} exiting={exiting} className={containerClassName} style={[style, styles?.container]} {...restProps}>
        {renderedChildren}
      </Animated.View>);
});
// --------------------------------------------------
