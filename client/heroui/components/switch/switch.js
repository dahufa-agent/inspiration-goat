...
import { SwitchAnimationProvider, useSwitchRootAnimation, useSwitchThumbAnimation, } from './switch.animation';
...
const Switch = forwardRef((props, ref) => {
    const { children, isDisabled, isSelected, onSelectedChange, className, style, animation, isAnimatedStyleActive = true, onPressIn, onPressOut, ...restProps } = props;
    const rootClassName = switchClassNames.root({
        isDisabled,
        className,
    });
    const { rContainerStyle, isSwitchPressed, contentContainerWidth, isAllAnimationsDisabled, } = useSwitchRootAnimation({
        animation,
        isSelected,
    });
    ...
    return (<SwitchProvider value={contextValue}>
        <AnimationSettingsProvider value={animationSettingsContextValue}>
          <SwitchAnimationProvider value={animationContextValue}>
            <AnimatedSwitchRoot ref={ref} className={rootClassName} style={rootStyle} isSelected={isSelected} onSelectedChange={onSelectedChange} isDisabled={isDisabled} onPressIn={handlePressIn} onPressOut={handlePressOut} onLayout={(e) => {
...
const SwitchThumb = forwardRef((props, ref) => {
    ...
    return (<AnimatedSwitchThumb ref={ref} className={thumbClassName} style={thumbStyle} {...restProps}>
      {content}
    </AnimatedSwitchThumb>);
});
// --------------------------------------------------
const SwitchStartContent = forwardRef((props, ref) => {
    const { children, className, ...restProps } = props;
    const startContentClassName = switchClassNames.startContent({
        className,
    });
    return (<View ref={ref} className={startContentClassName} {...restProps}>
        {children}
      </View>);
});
// --------------------------------------------------
const SwitchEndContent = forwardRef((props, ref) => {
    const { children, className, ...restProps } = props;
    const endContentClassName = switchClassNames.endContent({
        className,
    });
    return (<View ref={ref} className={endContentClassName} {...restProps}>
      {children}
    </View>);
});
// --------------------------------------------------
