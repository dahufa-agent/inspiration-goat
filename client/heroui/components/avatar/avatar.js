...
import { useAvatarFallbackAnimation, useAvatarImageAnimation, useAvatarRootAnimation, } from './avatar.animation';
import { AVATAR_DEFAULT_ICON_SIZE, AVATAR_DISPLAY_NAME, } from './avatar.constants';
...
const AvatarRoot = forwardRef((props, ref) => {
    const { children, size = 'md', variant = 'default', color = 'accent', className, style, animation, ...restProps } = props;
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
      <AvatarProvider value={contextValue}>
        <AvatarPrimitives.Root ref={ref} className={rootClassName} style={[avatarStyleSheet.borderCurve, style]} {...restProps}>
          {children}
...
const AvatarImage = forwardRef((props, ref) => {
    ...
    const imageStyle = isAnimatedStyleActive
        ...
        : styleProp;
    if (asChild) {
        return (<AvatarPrimitives.Image ref={ref} source={source} className={imageClassName} style={styleProp} asChild {...restProps}/>);
    }
    return (<AvatarPrimitives.Image ref={ref} source={source} asChild>
...
const DefaultFallbackIcon = ({ sizeVariant, colorVariant, iconProps }) => {
    const [themeColorDefaultForeground, themeColorAccent, themeColorSuccess, themeColorWarning, themeColorDanger,] = useThemeColor([
        'default-foreground',
        'accent',
        'success',
        'warning',
...
const AvatarFallback = forwardRef((props, ref) => {
    ...
    const { children, color: colorProp, className, classNames, style, styles, textProps, iconProps, delayMs, animation, ...restProps } = props;
    ...
    return (<AnimatedFallback key={AVATAR_DISPLAY_NAME.FALLBACK} ref={ref} entering={entering} className={fallbackContainerClassName} style={[avatarStyleSheet.borderCurve, style, styles?.container]} {...restProps}>
        {children ? (stringifiedChildren ? (<HeroText className={fallbackTextClassName} style={styles?.text} {...textProps}>
              {stringifiedChildren}
            </HeroText>) : (children)) : (<DefaultFallbackIcon sizeVariant={size} colorVariant={color} iconProps={iconProps}/>)}
      </AnimatedFallback>);
});
// --------------------------------------------------
