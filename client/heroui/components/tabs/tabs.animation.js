...
export function useTabsIndicatorAnimation(options) {
    ...
    const getPropertyConfig = (propertyAnimation) => {
        ...
        if (propertyAnimation.type === 'spring') {
            return {
                type: 'spring',
                springConfig: propertyAnimation.config ?? DEFAULT_INDICATOR_SPRING_CONFIG,
                timingConfig: { duration: 200 },
            };
        }
        return {
    ...
    const rContainerStyle = useAnimatedStyle(() => {
        ...
        const translateXAnimation = translateXConfig.type === 'timing'
