...
import { useTabsIndicatorAnimation, useTabsRootAnimation, useTabsSeparatorAnimation, } from './tabs.animation';
...
const TabsRoot = forwardRef((props, ref) => {
    const { children, value, onValueChange, className, variant = 'primary', animation, ...restProps } = props;
    const [measurements, setMeasurementsState] = useState({});
    const [isScrollView, setIsScrollView] = useState(false);
    const setMeasurements = useCallback((key, newMeasurements) => {
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
        <MeasurementsContext.Provider value={{
          ...
          <TabsPrimitives.Root ref={ref} value={value} onValueChange={onValueChange} className={rootClassName} {...restProps}>
            {children}
          </TabsPrimitives.Root>
...
const TabsList = forwardRef((props, ref) => {
    ...
    const handleLayout = useCallback(() => {
        ...
        const hasScrollView = childrenArray.length === 1 &&
            isValidElement(childrenArray[0]) &&
            childrenArray[0].type?.displayName ===
    ...
    return (<TabsPrimitives.List ref={ref} className={listClassName} style={[tabsStyleSheet.listRoot, style]} onLayout={handleLayout} {...restProps}>
        {children}
      </TabsPrimitives.List>);
});
// --------------------------------------------------
const TabsScrollView = forwardRef((props, ref) => {
    const { children, className, contentContainerClassName, showsHorizontalScrollIndicator = false, scrollAlign = 'center', ...restProps } = props;
    const { value } = useTabs();
    ...
    useEffect(() => {
        if (scrollAlign === 'none' || !measurements[value])
            return;
    ...
    return (<ScrollView ref={(instance) => {
        ...
        }} horizontal showsHorizontalScrollIndicator={showsHorizontalScrollIndicator} className={scrollViewClassName} contentContainerClassName={contentContainerClassNameValue} {...restProps}>
        {children}
      </ScrollView>);
});
// --------------------------------------------------
const TabsTrigger = forwardRef((props, ref) => {
    ...
    return (<TabsPrimitives.Trigger ref={ref} value={value} disabled={isDisabled} className={triggerClassName} style={[tabsStyleSheet.triggerRoot, style]} onLayout={handleLayout} {...restProps}>
      {content}
    </TabsPrimitives.Trigger>);
});
// --------------------------------------------------
const TabsLabel = forwardRef((props, ref) => {
    const { children, className, ...restProps } = props;
    const { isSelected } = useTabsTrigger();
    const labelClassName = tabsClassNames.label({ isSelected, className });
    return (<TabsPrimitives.Label ref={ref} className={labelClassName} {...restProps}>
        {children}
      </TabsPrimitives.Label>);
});
// --------------------------------------------------
const TabsIndicator = forwardRef((props, ref) => {
    const { children, className, style, animation, isAnimatedStyleActive = true, ...restProps } = props;
    ...
    const indicatorStyle = isAnimatedStyleActive
        ...
        : style;
    return (<AnimatedIndicator ref={ref} className={indicatorClassName} style={indicatorStyle} {...restProps}>
      {children}
    </AnimatedIndicator>);
});
// --------------------------------------------------
const TabsSeparator = forwardRef((props, ref) => {
    const { betweenValues, isAlwaysVisible = false, animation, isAnimatedStyleActive = true, className, style, ...restProps } = props;
    ...
    const separatorStyle = isAnimatedStyleActive
        ? [rContainerStyle, style]
        : style;
    return (<Animated.View ref={ref} className={separatorClassName} style={separatorStyle} {...restProps}/>);
});
// --------------------------------------------------
const TabsContent = forwardRef((props, ref) => {
    ...
    return (<TabsPrimitives.Content ref={ref} value={value} className={contentClassName} {...restProps}>
      {children}
