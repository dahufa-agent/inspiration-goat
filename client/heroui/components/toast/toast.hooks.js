...
export function useVerticalPlaceholderStyles({ rootClassName, style, }) {
    ...
    return useMemo(() => {
        ...
        const stylePaddingTop = resolvedStyle?.paddingTop ??
            resolvedStyle?.paddingVertical ??
            resolvedStyle?.padding;
        const stylePaddingBottom = resolvedStyle?.paddingBottom ??
            resolvedStyle?.paddingVertical ??
            resolvedStyle?.padding;
        const classNamePaddingTop = resolvedRootClassName?.paddingTop ??
            resolvedRootClassName?.paddingVertical ??
            resolvedRootClassName?.padding;
        const classNamePaddingBottom = resolvedRootClassName?.paddingBottom ??
            resolvedRootClassName?.paddingVertical ??
            resolvedRootClassName?.padding;
        ...
        const topHeight = typeof paddingTopValue === 'number' && paddingTopValue >= 0
            ? paddingTopValue
            : 0;
        const bottomHeight = typeof paddingBottomValue === 'number' && paddingBottomValue >= 0
            ? paddingBottomValue
            : 0;
