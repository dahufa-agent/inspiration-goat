...
const ToastRoot = forwardRef((props, ref) => {
    ...
    const { children, variant: localVariant, placement: localPlacement, index, total, heights, maxVisibleToasts, className, style, animation: localAnimation, isSwipeable: localIsSwipeable, isAnimatedStyleActive = true, hide, ...restProps } = props;
    /**
    ...
    const { rContainerStyle, entering, exiting, panGesture, isAllAnimationsDisabled, } = useToastRootAnimation({
        animation,
        index,
        total,
        heights,
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
      <ToastProvider value={contextValue}>
        <GestureDetector gesture={panGesture}>
          <Animated.View className={cn('absolute left-0 right-0', placement === 'top' ? 'top-0' : 'bottom-0')} entering={entering} exiting={exiting}>
        ...
        */}
            ...
            <AnimatedToastRoot pointerEvents="none" className={cn(rootClassName, 'absolute opacity-0')} style={[toastStyleSheet.root, style]} onLayout={(event) => {
...
const ToastTitle = forwardRef((props, ref) => {
    ...
    return (<HeroText ref={ref} className={labelClassName} {...restProps}>
      {children}
    </HeroText>);
});
// --------------------------------------------------
...
// --------------------------------------------------
const ToastAction = forwardRef((props, ref) => {
    ...
    const [themeColorDefaultHover, themeColorAccentHover, themeColorSuccessHover, themeColorWarningHover, themeColorDangerHover,] = useThemeColor([
        'default-hover',
        'accent-hover',
        'success-hover',
        'warning-hover',
    ...
    return (<Button ref={ref} variant={buttonVariant} size={size} className={actionClassName} feedbackVariant="scale-highlight" animation={mergedAnimation} {...restProps}>
      {children}
    </Button>);
});
// --------------------------------------------------
const ToastClose = forwardRef((props, ref) => {
    ...
    return (<Button ref={ref} variant="ghost" size={size} isIconOnly aria-label="Close" className={className} onPress={handlePress} {...restProps}>
      {children ?? (<CloseIcon size={iconProps?.size ?? 16} color={iconProps?.color ?? themeColorMuted}/>)}
    </Button>);
});
// --------------------------------------------------
/**
...
export function DefaultToast(props) {
    const globalConfig = useToastConfig();
    const { id, variant: localVariant, placement: localPlacement, isSwipeable: localIsSwipeable, animation: localAnimation, label, description, actionLabel, onActionPress, icon, hide, show, ...toastComponentProps } = props;
    /**
     * Merge global config with local props, ensuring local props take precedence
     */
    const variant = localVariant ?? globalConfig?.variant ?? 'default';
    ...
    return (<ToastRoot id={id} variant={variant} placement={placement} isSwipeable={isSwipeable} animation={animation} className="flex-row gap-3" hide={hide} show={show} {...toastComponentProps}>
      {icon && <View>{icon}</View>}
