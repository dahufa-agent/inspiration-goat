...
import { Easing, Extrapolation, FadeInDown, FadeInUp, interpolate, Keyframe, useAnimatedStyle, useSharedValue, withDecay, withSpring, withTiming, } from 'react-native-reanimated';
import { scheduleOnRN } from 'react-native-worklets';
import { useCombinedAnimationDisabledState } from '../../helpers/internal/hooks';
import { getAnimationValueMergedConfig, getAnimationValueProperty, getIsAnimationDisabledValue, getRootAnimationState, } from '../../helpers/internal/utils';
// --------------------------------------------------
...
export function useToastRootAnimation(options) {
    const { animation, index, total, heights, placement, hide, id, isSwipeable = true, maxVisibleToasts = 3, } = options;
    ...
    const delayedHide = (toastId, velocity) => {
    ...
    const panGesture = Gesture.Pan()
        ...
        if (placement === 'top') {
            ...
            if (translationY < 0) {
                ...
                const rubberTranslateY = interpolate(translationY, [0, maxDragDistance], [0, rubberEffectDistance], Extrapolation.CLAMP);
            ...
            if (translationY > 0) {
                ...
                const rubberTranslateY = interpolate(Math.abs(translationY), [0, maxDragDistance], [0, rubberEffectDistance], Extrapolation.CLAMP);
                gestureTranslateY.set(-rubberTranslateY);
            }
        }
    })
        ...
        const shouldDismiss = Math.abs(translationY) > dismissThreshold ||
        ...
        if (placement === 'top') {
            ...
            if (shouldDismiss && translationY < 0 && id) {
                ...
                gestureTranslateY.set(withDecay({
