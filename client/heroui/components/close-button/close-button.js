...
const CloseButtonRoot = forwardRef((props, ref) => {
    ...
    return (<Button ref={ref} variant="tertiary" size="sm" isIconOnly className={rootClassName} hitSlop={12} {...restProps}>
        {children ?? (<CloseIcon size={iconProps?.size ?? 18} color={iconProps?.color ?? themeColorMuted}/>)}
      </Button>);
});
// --------------------------------------------------
