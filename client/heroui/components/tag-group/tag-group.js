...
import { useItemContext as usePrimitiveItemContext, useRootContext as usePrimitiveRootContext, } from '../../primitives/tag-group';
...
// --------------------------------------------------
const TagGroupRoot = forwardRef((props, ref) => {
    const { children, size = 'md', variant = 'default', className, style, animation, isDisabled = false, isInvalid = false, isRequired = false, ...restProps } = props;
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
      <FormFieldProvider value={formFieldContextValue}>
        <TagGroupProvider value={contextValue}>
          <TagGroupPrimitives.Root ref={ref} className={rootClassName} style={style} isDisabled={isDisabled} {...restProps}>
            {children}
...
// --------------------------------------------------
const TagGroupList = forwardRef((props, ref) => {
    const { children, className, style, renderEmptyState, ...restProps } = props;
    const listClassName = tagGroupClassNames.list({
        className,
    });
    const hasChildren = Children.count(children) > 0;
    return (<TagGroupPrimitives.List ref={ref} className={listClassName} style={style} {...restProps}>
...
const TagGroupItem = forwardRef((props, ref) => {
    ...
    if (typeof children === 'function') {
        ...
        return (<TagGroupPrimitives.Item ref={ref} id={id} isDisabled={isDisabledProp} className={tagClassName} style={[tagGroupStyleSheet.tag, style]} {...restProps}>
          {children(renderProps)}
        </TagGroupPrimitives.Item>);
    }
    const stringifiedChildren = childrenToString(children);
    return (<TagGroupPrimitives.Item ref={ref} id={id} isDisabled={isDisabledProp} className={tagClassName} style={[tagGroupStyleSheet.tag, style]} {...restProps}>
        {stringifiedChildren ? (<TagGroupItemLabel>{stringifiedChildren}</TagGroupItemLabel>) : (children)}
      </TagGroupPrimitives.Item>);
});
// --------------------------------------------------
const TagGroupItemLabel = forwardRef((props, ref) => {
    ...
    return (<TagGroupPrimitives.ItemLabel asChild>
...
// --------------------------------------------------
const TagGroupItemRemoveButton = forwardRef((props, ref) => {
    ...
    const defaultIcon = (<CloseIcon size={iconProps?.size ?? 12} color={iconProps?.color ?? defaultIconColor}/>);
    return (<TagGroupPrimitives.RemoveButton ref={ref} className={removeButtonClassName} hitSlop={hitSlop} {...restProps}>
