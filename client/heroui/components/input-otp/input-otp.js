...
import { useInputOTPRootAnimation, useInputOTPSlotCaretAnimation, useInputOTPSlotValueAnimation, } from './input-otp.animation';
...
const InputOTPRoot = forwardRef((props, ref) => {
    const { className, animation, isBottomSheetAware, onFocus: onFocusProp, onBlur: onBlurProp, ...restProps } = props;
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
        <InputOTPPrimitives.Root ref={ref} className={rootClassName} onFocus={onFocus} onBlur={onBlur} {...restProps}/>
...
const InputOTPGroup = forwardRef((props, ref) => {
    ...
    return (<InputOTPPrimitives.Group ref={ref} className={groupClassName} {...restProps}>
        {content}
...
const InputOTPSlot = forwardRef((props, ref) => {
    ...
    const finalVariant = variant !== undefined
        ? variant
    ...
    return (<InputOTPSlotProvider value={slotContextValue}>
        <InputOTPPrimitives.Slot ref={ref} index={index} className={slotClassName} style={[inputOTPStyleSheet.slotRoot, style]} {...restProps}>
 ...
 */
const InputOTPSlotPlaceholder = forwardRef((props, ref) => {
    ...
    return (<HeroText ref={ref} className={slotPlaceholderClassName} style={[slotPlaceholderTextStyle, style]} {...restProps}>
      {displayChar}
    </HeroText>);
});
// --------------------------------------------------
 ...
 */
const InputOTPSlotValue = forwardRef((props, ref) => {
    const { className, children, animation, ...restProps } = props;
    const { slot } = useInputOTPSlot();
    const displayChar = children ?? slot?.char ?? '';
    const { wrapperEntering, wrapperExiting, textEntering, textExiting } = useInputOTPSlotValueAnimation({
        animation,
    });
    ...
    return (<Animated.View entering={wrapperEntering} exiting={wrapperExiting}>
      <AnimatedText ref={ref} entering={textEntering} exiting={textExiting} className={slotValueClassName} {...restProps}>
        {displayChar}
      </AnimatedText>
    </Animated.View>);
});
// --------------------------------------------------
const InputOTPSlotCaret = forwardRef((props, ref) => {
    const { className, animation, isAnimatedStyleActive = true, pointerEvents = 'none', style, ...restProps } = props;
    ...
    if (!isCaretVisible)
        return null;
    return (<Animated.View ref={ref} className={slotCaretClassName} style={containerStyle} pointerEvents={pointerEvents} {...restProps}/>);
});
// --------------------------------------------------
