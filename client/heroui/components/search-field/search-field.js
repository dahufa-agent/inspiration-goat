...
// --------------------------------------------------
const SearchFieldRoot = forwardRef((props, ref) => {
    const { children, className, value, onChange, isDisabled = false, isInvalid = false, isRequired = false, animation, ...restProps } = props;
    const rootClassName = searchFieldClassNames.root({ className });
    const { isAllAnimationsDisabled } = useSearchFieldRootAnimation({
        animation,
    });
    const searchFieldContextValue = useMemo(() => ({ value, onChange, isDisabled, isInvalid, isRequired }), [value, onChange, isDisabled, isInvalid, isRequired]);
    const formFieldContextValue = useMemo(() => ({ isDisabled, isInvalid, isRequired, hasFieldPadding: true }), [isDisabled, isInvalid, isRequired]);
    const animationSettingsContextValue = useMemo(() => ({
        isAllAnimationsDisabled,
    }), [isAllAnimationsDisabled]);
    return (<SearchFieldProvider value={searchFieldContextValue}>
...
// --------------------------------------------------
const SearchFieldGroup = forwardRef((props, ref) => {
    const { children, className, ...restProps } = props;
    const groupClassName = searchFieldClassNames.group({ className });
    return (<View ref={ref} className={groupClassName} {...restProps}>
        {children}
      </View>);
});
// --------------------------------------------------
const SearchFieldSearchIcon = forwardRef((props, ref) => {
    const { children, className, iconProps, ...restProps } = props;
    const searchIconClassName = searchFieldClassNames.searchIcon({ className });
    return (<View ref={ref} className={searchIconClassName} pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants" {...restProps}>
        {children ?? (<SearchIcon size={iconProps?.size} color={iconProps?.color}/>)}
      </View>);
});
// --------------------------------------------------
const SearchFieldInput = forwardRef((props, ref) => {
    const { className, placeholder = 'Search...', returnKeyType = 'search', accessibilityRole = 'search', accessibilityLabel = 'Search', ...restProps } = props;
    const searchField = useSearchField();
    const inputClassName = searchFieldClassNames.input({ className });
    return (<Input ref={ref} className={inputClassName} value={searchField?.value} onChangeText={searchField?.onChange} placeholder={placeholder} returnKeyType={returnKeyType} accessibilityRole={accessibilityRole} accessibilityLabel={accessibilityLabel} {...restProps}/>);
});
// --------------------------------------------------
const SearchFieldClearButton = forwardRef((props, ref) => {
    ...
    return (<Button ref={ref} variant="tertiary" size="sm" isIconOnly className={clearButtonClassName} hitSlop={8} accessibilityRole="button" accessibilityLabel="Clear search" onPress={handlePress} {...restProps}>
        {children ?? (<CloseIcon size={iconProps?.size ?? 14} color={iconProps?.color ?? themeColorMuted}/>)}
      </Button>);
});
// --------------------------------------------------
