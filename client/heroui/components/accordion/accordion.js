...
import { AccordionAnimationProvider, useAccordionAnimation, useAccordionContentAnimation, useAccordionIndicatorAnimation, useAccordionRootAnimation, } from './accordion.animation';
...
const Root = forwardRef((props, ref) => {
    const { children, variant = 'default', hideSeparator = false, className, classNames, styles, style, animation, ...restProps } = props;
    ...
    return (<AnimationSettingsProvider value={animationSettingsContextValue}>
      <AccordionAnimationProvider value={animationContextValue}>
        <AccordionInnerProvider value={contextValue}>
          <AnimatedRootView ref={ref} className={containerClassName} style={[accordionStyleSheet.root, style, styles?.container]} layout={layoutTransition} {...restProps}>
            {Children.map(children, (child, index) => (<>
                {child}
                {!hideSeparator && index < Children.count(children) - 1 && (<Animated.View className={separatorClassName} style={styles?.separator} layout={layoutTransition}/>)}
              </>))}
          </AnimatedRootView>
...
const Item = forwardRef((props, ref) => {
    const { children, value, layout: layoutProp, className, isDisabled: isDisabledProp, ...restProps } = props;
    ...
    return (<AnimatedItemView ref={ref} layout={layoutProp || layoutTransition} value={value} className={itemClassName} isDisabled={isDisabledProp} {...restProps}>
      {content}
    </AnimatedItemView>);
});
// ------------------------------------------------------------------------------
...
const Indicator = forwardRef((props, ref) => {
    ...
    const indicatorStyle = isAnimatedStyleActive
        ...
        : style;
    if (children) {
        return (<AnimatedIndicator ref={ref} className={indicatorClassName} style={style} {...restProps}>
        {children}
      </AnimatedIndicator>);
    }
...
const Content = forwardRef((props, ref) => {
    ...
    return (<Animated.View entering={animatedEntering} exiting={animatedExiting}>
