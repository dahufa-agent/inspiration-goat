...
import { DEFAULT_SCROLL_EVENT_THROTTLE, DEFAULT_SHADOW_SIZE, SCROLL_SHADOW_DISPLAY_NAME, } from './scroll-shadow.constants';
import { scrollShadowClassNames, scrollShadowStyleSheet, } from './scroll-shadow.styles';
/**
...
const ScrollShadowRoot = forwardRef((props, ref) => {
    const { children, size = DEFAULT_SHADOW_SIZE, orientation: orientationProp, visibility = 'auto', color, isEnabled = true, className, style, LinearGradientComponent, animation, ...restProps } = props;
    const themeColorBackground = useThemeColor('background');
    const shadowColor = color || themeColorBackground;
    const rootClassName = scrollShadowClassNames.root({ className });
    const childHorizontal = children?.props &&
    ...
    const { contentSize, containerSize, localScrollHandler, topShadowStyle, bottomShadowStyle, } = useScrollShadowRootAnimation({
        animation,
        orientation,
        size,
        visibility,
    ...
    const scrollEventThrottle = children.props?.scrollEventThrottle ||
        DEFAULT_SCROLL_EVENT_THROTTLE;
    if (!isValidElement(children)) {
        return null;
    }
    const isAnimatedComponent = children.type?.displayName?.includes('AnimatedComponent') ||
        children.type?.__isAnimatedComponent;
    const enhancedChild = isAnimatedComponent
        ...
        : createElement(getAnimatedComponent(children.type), {
            ...children.props,
            onContentSizeChange,
            onLayout,
            scrollEventThrottle,
    ...
    return (<View ref={ref} className={rootClassName} style={style} {...restProps}>
      {enhancedChild}

      {/* Top/Left Shadow */}
      {orientation === 'vertical' ? (<Animated.View style={[
                scrollShadowStyleSheet.topShadow,
                { height: size },
                topShadowStyle,
            ]}>
          <LinearGradientComponent colors={topLeftColors} locations={topLeftLocations} style={StyleSheet.absoluteFill}/>
        </Animated.View>) : (<Animated.View style={[
                scrollShadowStyleSheet.leftShadow,
                { width: size },
                topShadowStyle,
            ]}>
          <LinearGradientComponent colors={topLeftColors} locations={topLeftLocations} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={StyleSheet.absoluteFill}/>
        </Animated.View>)}
      ...
      {orientation === 'vertical' ? (<Animated.View style={[
          ...
          <LinearGradientComponent colors={bottomRightColors} locations={bottomRightLocations} style={StyleSheet.absoluteFill}/>
        </Animated.View>) : (<Animated.View style={[
                scrollShadowStyleSheet.rightShadow,
                { width: size },
                bottomShadowStyle,
            ]}>
          <LinearGradientComponent colors={bottomRightColors} locations={bottomRightLocations} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={StyleSheet.absoluteFill}/>
        </Animated.View>)}
    </View>);
});
