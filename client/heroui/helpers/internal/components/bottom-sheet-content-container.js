 ...
 *
 */
export function BottomSheetContentContainer({ children, isOpen, progress, isDragging, isPanActivated, isClosingOnSwipe, initialIndex, contentContainerClassName, contentContainerProps, onOpenChange, }) {
    const { close, snapToIndex } = useBottomSheet();
    const prevIsOpenRef = useRef(isOpen);
    const closeBottomSheet = () => {
        onOpenChange(false);
    };
    useAnimatedReaction(() => progress.get(), (value) => {
    ...
    return (<BottomSheetView className={contentContainerClassName} {...contentContainerProps}>
      {children}
    </BottomSheetView>);
}
