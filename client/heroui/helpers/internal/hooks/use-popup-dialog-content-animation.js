...
import { Easing, Extrapolation, interpolate, Keyframe, useAnimatedStyle, useDerivedValue, useSharedValue, withSpring, } from 'react-native-reanimated';
 ...
 */
const DEFAULT_ENTERING_ANIMATION = new Keyframe({
    0: {
 ...
 */
const DEFAULT_EXITING_ANIMATION = new Keyframe({
    0: {
        transform: [{ scale: 1 }],
        opacity: 1,
    },
...
export const usePopupDialogContentAnimation = ({ isOpen, progress, isDragging, isGestureReleaseAnimationRunning, onOpenChange, animation, isSwipeable = true, }) => {
    ...
    const contentTranslateY = useDerivedValue(() => {
        ...
        return interpolate(absoluteGestureTranslationY, [0, screenHeight], [0, -50], Extrapolation.CLAMP);
    });
    ...
    const rDragContainerStyle = useAnimatedStyle(() => {
        if (isGestureReleaseAnimationRunning.get()) {
            return {
                ...
                transform: [
                    {
                        translateY: interpolate(progress.get(), [
                            progressAnchor.get(),
                            1,
                            progressAnchor.get(),
