...
export const colorKitUI = () => {
    ...
    const RGB_string_to_object = (color) => {
        ...
        const r = parseInt(matches[1] || '0', 10), g = parseInt(matches[2] || '0', 10), b = parseInt(matches[3] || '0', 10), a = parseFloat(matches[4] ?? '1');
        return {
            r: clampRGB(r),
            g: clampRGB(g),
            b: clampRGB(b),
    ...
    const RGB_to_HEX = (color) => {
        ...
        const red = numberToHexString(r), green = numberToHexString(g), blue = numberToHexString(b), alpha = a === 1 ? '' : numberToHexString(a * 255);
    ...
    const RGB_to_HSLA = (color) => {
        const rgb = typeof color === 'string'
            ? RGB_string_to_object(color)
            : normalize_RGB_object(color), r = rgb.r / 255, g = rgb.g / 255, b = rgb.b / 255, a = rgb.a;
    ...
    const RGB_to_HSVA = (color) => {
        const rgb = typeof color === 'string'
            ? RGB_string_to_object(color)
            : normalize_RGB_object(color), r = rgb.r / 255, g = rgb.g / 255, b = rgb.b / 255, a = rgb.a;
        const max = Math.max(r, g, b), min = Math.min(r, g, b), d = max - min, v = max, s = max === 0 ? 0 : d / max;
        let h = 0;
        if (max === min) {
            h = 0;
        }
    ...
    const RGB_to_HWBA = (color) => {
        const rgb = typeof color === 'string'
            ? RGB_string_to_object(color)
            : normalize_RGB_object(color), red = rgb.r / 255, green = rgb.g / 255, blue = rgb.b / 255, a = rgb.a;
    ...
    const HSL_string_to_object = (color) => {
        ...
        const h = parseInt(matches[1] || '0', 10), s = parseInt(matches[2] || '0', 10), l = parseInt(matches[3] || '0', 10), a = parseFloat(matches[4] ?? '1');
        return {
            h: clampHue(h),
            s: clamp100(s),
            l: clamp100(l),
    ...
    const HSL_to_RGBA = (color) => {
        const hsla = typeof color === 'string'
            ? HSL_string_to_object(color)
            : normalize_HSL_object(color);
        const h = hsla.h / 360, s = hsla.s / 100, l = hsla.l / 100, a = hsla.a;
        const q = l < 0.5 ? l * (1 + s) : l + s - l * s, p = 2 * l - q;
        const r = calculateHueValue(p, q, h + 1 / 3), g = calculateHueValue(p, q, h), b = calculateHueValue(p, q, h - 1 / 3);
        return {
            r: clampRGB(r * 255),
            g: clampRGB(g * 255),
            b: clampRGB(b * 255),
    ...
    const HSL_to_HEX = (color) => {
        ...
        const r = numberToHexString(rgb.r), g = numberToHexString(rgb.g), b = numberToHexString(rgb.b), a = rgb.a === 1 ? '' : numberToHexString(rgb.a * 255);
        return `#${r + g + b + a}`;
    };
    ...
    const HSL_to_HSVA = (color) => {
        const hsla = typeof color === 'string'
            ...
            : normalize_HSL_object(color);
        const h = hsla.h;
        const s = hsla.s / 100, l = hsla.l / 100, a = hsla.a ?? 1, v = l + s * Math.min(l, 1 - l), sNew = v === 0 ? 0 : 2 - (2 * l) / v;
        return {
            h: clampHue(h),
            s: clamp100(sNew * 100),
            v: clamp100(v * 100),
    ...
    const HWB_string_to_object = (color) => {
        ...
        const h = parseInt(matches[1] || '0', 10), w = parseInt(matches[2] || '0', 10), b = parseInt(matches[3] || '0', 10), a = parseFloat(matches[4] ?? '1');
        return {
            h: clampHue(h),
            w: clamp100(w),
            b: clamp100(b),
    ...
    const HWB_to_RGBA = (color) => {
        const hwba = typeof color === 'string'
            ? HWB_string_to_object(color)
            : normalize_HWB_object(color);
        const h = hwba.h / 360, w = hwba.w / 100, b = hwba.b / 100, a = hwba.a;
        if (w + b >= 1) {
        ...
        const red = calculateHueValue(0, 1, h + 1 / 3) * (1 - w - b) + w, green = calculateHueValue(0, 1, h) * (1 - w - b) + w, blue = calculateHueValue(0, 1, h - 1 / 3) * (1 - w - b) + w;
        return {
            r: clampRGB(red * 255),
    ...
    const HSV_string_to_object = (color) => {
        ...
        const h = parseInt(matches[1] || '0', 10), s = parseInt(matches[2] || '0', 10), v = parseInt(matches[3] || '0', 10), a = parseFloat(matches[4] ?? '1');
        return {
            h: clampHue(h),
            s: clamp100(s),
            v: clamp100(v),
    ...
    const HSV_to_RGBA = (color) => {
        ...
        const h = hsva.h / 360, s = hsva.s / 100, v = hsva.v / 100, a = hsva.a;
    ...
    const HSV_to_HSLA = (color) => {
        const hsva = typeof color === 'string'
            ? HSV_string_to_object(color)
            : normalize_HSV_object(color);
        const h = hsva.h, s = hsva.s / 100, v = hsva.v / 100, a = hsva.a;
        const l = ((2 - s) * v) / 2, sl = s * v, sln = l !== 0 && l !== 1 ? sl / (l < 0.5 ? l * 2 : 2 - l * 2) : sl;
        return {
            h: clampHue(h),
            s: clamp100(sln * 100),
            l: clamp100(l * 100),
    ...
    const HSV_to_HWBA = (color) => {
        const { h, s, v, a } = typeof color === 'string'
            ? HSV_string_to_object(color)
            : normalize_HSV_object(color);
        const w = (1 - s / 100) * v, b = (1 - v / 100) * 100;
        return {
            h: clampHue(h),
            w: clamp100(w),
            b: clamp100(b),
    ...
    const normalize_HEX = (color) => {
        ...
        if (hex.length === 3)
            return `#${hex.map((x) => x + x).join('')}ff`;
        if (hex.length === 4)
            return `#${hex.map((x) => x + x).join('')}`;
        if (hex.length === 6)
            return `#${hex.join('')}ff`;
        return color;
    };
    ...
    const HEX_to_RGBA = (color) => {
        ...
        const r = parseInt(matches[1] || '0', 16), g = parseInt(matches[2] || '0', 16), b = parseInt(matches[3] || '0', 16), a = parseInt(matches[4] || 'ff', 16) / 255;
        return {
            r: clampRGB(r),
            g: clampRGB(g),
            b: clampRGB(b),
    ...
    const getLuminanceWCAG = (color) => {
        ...
        const a = [r, g, b].map((v) => v / 255 <= 0.03928
            ? v / 255 / 12.92
    ...
    const increaseRed = (color, amount) => {
        ...
        const red = typeof amount === 'string'
    ...
    const increaseGreen = (color, amount) => {
        const { r, g, b, a } = RGB(color).object();
        const green = typeof amount === 'string'
    ...
    const increaseBlue = (color, amount) => {
        const { r, g, b, a } = RGB(color).object();
        const blue = typeof amount === 'string'
    ...
    const increaseAlpha = (color, amount) => {
        const { r, g, b, a } = RGB(color).object();
        const alpha = typeof amount === 'string'
    ...
    const increaseHue = (color, amount) => {
        ...
        const hue = typeof amount === 'string'
    ...
    const saturate = (color, amount) => {
        const { h, s, l, a } = HSL(color).object();
        const saturation = typeof amount === 'string'
    ...
    const brighten = (color, amount) => {
        const { h, s, l, a } = HSL(color).object();
        const lum = typeof amount === 'string'
    ...
    const blend = (color1, color2, percentage) => {
        ...
        const r = clampRGB(rgba1.r * (1 - percentage) + rgba2.r * percentage), g = clampRGB(rgba1.g * (1 - percentage) + rgba2.g * percentage), b = clampRGB(rgba1.b * (1 - percentage) + rgba2.b * percentage), a = clampAlpha(rgba1.a * (1 - percentage) + rgba2.a * percentage);
    ...
    const randomHslColor = ({ h = [0, 360], s = [0, 100], l = [0, 100], a = [1, 1], } = {}) => {
        const random = {
    ...
    const randomHsvColor = ({ h = [0, 360], s = [0, 100], v = [0, 100], a = [1, 1], } = {}) => {
        const random = {
            h: clampHue(randomNumber(h[0] ?? 0, h[1] ?? 360)),
            s: clamp100(randomNumber(s[0] ?? 0, s[1] ?? 100)),
    ...
    const randomRgbColor = ({ r = [0, 255], g = [0, 255], b = [0, 255], a = [1, 1], } = {}) => {
        const random = {
    ...
    const randomHwbColor = ({ h = [0, 360], w = [0, 100], b = [0, 100], a = [1, 1], } = {}) => {
        const random = {
            h: clampHue(randomNumber(h[0] ?? 0, h[1] ?? 360)),
    ...
    const adjustContrast = (color1, color2, ratio = 4.5) => {
        ...
        if (ratio && contrast < ratio) {
            while (contrastRatio(newColor, color2) < ratio) {
                ...
                if (channels.every((e) => newColor[e] === 0))
                    break;
                if (channels.every((e) => newColor[e] === 255))
                    break;
            }
            //* decrease contrast
        }
