...
function createStringToastComponent(label, globalConfig) {
    return (props) => {
        ...
        return (<DefaultToast {...props} label={label} variant={mergedConfig.variant} placement={mergedConfig.placement} isSwipeable={mergedConfig.isSwipeable} animation={mergedConfig.animation}/>);
    };
}
/**
...
function createConfigToastComponent(config, globalConfig) {
    return (props) => {
        ...
        return (<DefaultToast {...props} variant={mergedConfig.variant} placement={mergedConfig.placement} isSwipeable={mergedConfig.isSwipeable} animation={mergedConfig.animation} label={config.label} description={config.description} actionLabel={config.actionLabel} onActionPress={config.onActionPress} icon={config.icon}/>);
    };
}
/**
 ...
 */
export function ToastProvider({ defaultProps, insets, maxVisibleToasts = 3, contentWrapper, children, disableFullWindowOverlay = false, }) {
    ...
    return (<ToastConfigContext.Provider value={globalConfig}>
      <ToasterContext.Provider value={contextValue}>
        {children}
        {toasts.length > 0 && (<InsetsContainer insets={insets} contentWrapper={contentWrapper} disableFullWindowOverlay={disableFullWindowOverlay}>
            <View className="flex-1">
              {toasts.map((toastItem, index) => (<ToastItemRenderer key={toastItem.id} toastItem={toastItem} show={show} hide={hide} index={index} total={total} heights={heights} maxVisibleToasts={maxVisibleToasts}/>))}
            </View>
          </InsetsContainer>)}
