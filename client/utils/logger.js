...
class ErrorLogger {
    ...
    log(type, source, message, details) {
        const logEntry = {
            timestamp: Date.now(),
            type,
            source,
    ...
    network(source, message, details) {
    ...
    async syncToServer() {
        ...
        try {
            ...
            if (response.ok) {
                // 同步成功的日志移除
                this.logs = this.logs.filter((log) => !logsCopy.some((syncLog) => syncLog.timestamp === log.timestamp));
                this.saveLogs();
...
// 便捷方法
export const logError = (source, message, details) => errorLogger.error(source, message, details);
export const logNetwork = (source, message, details) => errorLogger.network(source, message, details);
